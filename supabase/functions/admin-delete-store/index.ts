...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // 11. Delete financial_transactions (which reference profiles via created_by)
    const { error: deleteFinancialTransactionsError } = await supabaseAdmin
      .from('financial_transactions')
      .delete()
      .eq('store_id', storeId);
    if (deleteFinancialTransactionsError) throw deleteFinancialTransactionsError;
    console.log(`Deleted financial transactions for store ${storeId}.`);

    // 12. Get users (profiles) linked to the store
    ...
    // 13. Delete users from Supabase Auth and profiles table
