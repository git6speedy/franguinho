...
export default function Totem() {
  ...
  // Formas de pagamento e máquinas
  ...
  const totalMonetary = Math.max(0, monetarySubtotal - couponDiscount); // Definido aqui para ser acessível a isOrderValid
  // ------------------------------------------------
  ...
  const loadProductsAndVariations = async () => {
    ...
    let productsQuery = supabase
      ...
      .select("id, name, price, ifood_price, stock_quantity, image_url, earns_loyalty_points, loyalty_points_value, has_variations, can_be_redeemed_with_points, redemption_points_cost, category_id")
      .eq("store_id", storeId)
      .eq("active", true)
      .order("name");
    ...
    // Aplicar preço ifood para totem não se aplica pois totem não tem seleção de canal
  ...
  const isOrderValid = () => {
    ...
    if (totalMonetary > 0 && !paymentMethod && !selectedPaymentMethodId) return false;
  ...
  const handleFinishOrderClick = () => {
    ...
    if (!isOrderValid()) {
      ...
      } else if (totalMonetary > 0 && !paymentMethod && !selectedPaymentMethodId) {
  ...
  const finishOrder = async (customerNameOverride?: string) => {
    ...
    let finalPaymentMethod = paymentMethodsArray.join(' + ');
                      ...
                      <div className="space-y-3">
                        ...
                        {((paymentMethod === "credito" || paymentMethod === "debito") || 
                         ...
                         cardMachines.length > 0 && (
                          <div className="space-y-2">
                            <Label htmlFor="cardMachine">Máquina de Cartão</Label>
