...
export default function Products() {
  ...
  const [editProductIfoodPrice, setEditProductIfoodPrice] = useState("");
  ...
  const loadProducts = async () => {
    ...
    const { data, error } = await supabase
      .from("products")
      .select("*, supplier_product_id, ifood_price") // Selecionar ifood_price
      .eq("store_id", profile.store_id)
      .order("name");
  ...
  const openEditProductDialog = async (product: Product) => {
    ...
    setEditProductIfoodPrice(product.ifood_price?.toString() || "");
  ...
  const handleUpdateProduct = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        ifood_price: editProductIfoodPrice ? parseFloat(editProductIfoodPrice) : null,
        ...
        <DialogContent className="max-h-[99vh] overflow-auto">
          ...
          {editingProduct && (
            <form onSubmit={handleUpdateProduct} className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="editProductIfoodPrice">Preço Ifood (R$)</Label>
                <Input
                  id="editProductIfoodPrice"
                  type="number"
                  step="0.01"
                  value={editProductIfoodPrice}
                  onChange={(e) => setEditProductIfoodPrice(e.target.value)}
                  placeholder="Opcional"
                  disabled={editProductHasVariations}
                />
                <p className="text-xs text-muted-foreground">
                  Se deixar vazio, usará o preço normal
                </p>
              </div>
              <div className="space-y-2">
