...
export default function PDV() {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // Estado para controlar submissão duplicada
  const [quickSaleMode, setQuickSaleMode] = useState<'off' | 'auto' | 'fixed'>('off'); // Modo venda rápida
  ...
  }, [profile, source]); // Adicionar source como dependência para recarregar quando mudar
  ...
  const loadProductsAndVariations = async () => {
    const { data: productsData, error: productsError } = await supabase
      .from("products")
      .select("id, name, price, ifood_price, stock_quantity, has_variations, earns_loyalty_points, loyalty_points_value, can_be_redeemed_with_points, redemption_points_cost, category_id")
      .eq("store_id", profile.store_id)
      .eq("active", true)
      .order("name");

    // Aplicar preço ifood se o canal for ifood
    if (source === "ifood" && productsData) {
      productsData.forEach((product: any) => {
        if (product.ifood_price && product.ifood_price > 0) {
          product.price = product.ifood_price;
        }
      });
    }
  ...
  const handleAddToCart = async (product: Product, variation?: Variation) => {
    // CORREÇÃO: Não exigir cliente no canal presencial
    // Apenas exigir telefone para whatsapp e ifood
  ...
  const totalMonetary = useMemo(() => {
    return Math.max(0, monetarySubtotal + finalDeliveryAmount - totalDiscountAmount);
  }, [monetarySubtotal, finalDeliveryAmount, totalDiscountAmount]); // Total a ser pago em dinheiro/cartão/pix
  ...
  const handlePrintOrder = (orderNumber: string) => {
    ...
    if (paymentMethod) {
      ...
      const customMethod = customPaymentMethods.find(pm => pm.id === selectedPaymentMethodId);
      paymentMethodName = customMethod?.name || '';
    }
    
    // Determinar o nome da maquininha
    let cardMachineName = '';
    if (selectedCardMachineId) {
      const selectedMachine = cardMachines.find(m => m.id === selectedCardMachineId);
      cardMachineName = selectedMachine?.name || '';
    }

    // Construir objeto de pedido para impressão
    const orderData = {
      order_number: orderNumber,
      created_at: new Date().toISOString(),
      customer_name: customerNameForOrder,
      customers: customerPhone ? { name: customerNameForOrder, phone: customerPhone } : undefined,
      payment_method: paymentMethodName,
      delivery: isDelivery,
      delivery_address: isDelivery ? address : undefined,
      delivery_number: isDelivery ? number : undefined,
      delivery_neighborhood: isDelivery ? neighborhood : undefined,
      delivery_reference: isDelivery ? reference : undefined,
      delivery_cep: isDelivery && !skipCep ? cep : undefined,
      delivery_fee: isDelivery ? deliveryAmount : undefined,
      pickup_time: isReserva ? reservaPickupTime : undefined,
      notes: notes || undefined,
      discount_amount: discountAmount,
      total: totalMonetary,
      change_for: paymentMethod === "dinheiro" && changeFor ? parseFloat(changeFor) : undefined,
      card_machine_name: cardMachineName || undefined,
    };

    // Construir array de itens para impressão
    const orderItems = cart.map(item => ({
      product_name: item.name,
      quantity: item.quantity,
      variation_name: item.selectedVariation?.name,
      product_price: item.isRedeemedWithPoints ? 0 : item.price,
      subtotal: item.isRedeemedWithPoints ? 0 : item.price * item.quantity,
    }));

    // Importar e chamar a função centralizada de impressão
    import('@/utils/printUtils').then(({ printOrder }) => {
  ...
  const finishOrder = async () => {
    try {
      ...
      const manualDiscountAmount = discount ? parseFloat(discount) : 0;
      const couponDiscountAmount = appliedCouponData?.discountAmount || 0;
      const totalDiscountAmount = manualDiscountAmount + couponDiscountAmount;
      // Apply free shipping from coupon
      const finalDeliveryAmount = appliedCouponData?.freeShipping ? 0 : deliveryAmount;
      const totalMonetary = Math.max(0, monetarySubtotal + finalDeliveryAmount - totalDiscountAmount); // Total a ser pago em dinheiro/cartão/pix
      ...
      if (totalMonetary > 0 && !paymentMethod && !selectedPaymentMethodId) {
        toast({
          variant: "destructive",
      ...
      // CORRIGIDO: Adicionar nome da forma de pagamento customizada
      if (totalMonetary > 0 && selectedPaymentMethodId) {
        const customMethod = customPaymentMethods.find(pm => pm.id === selectedPaymentMethodId);
        if (customMethod) {
          paymentMethodsArray.push(customMethod.name);
        }
      }
      const finalPaymentMethod = paymentMethodsArray.join(' + ');

      // Determinar o status inicial dinamicamente
      const initialStatus = quickSaleMode !== 'off' ? 'delivered' : getInitialStatus();
      
      // Se modo auto, volta para off após a venda
      if (quickSaleMode === 'auto') {
        setQuickSaleMode('off');
      }

      // Preparar dados de múltiplos pagamentos se aplicável
      let paymentMethodsJson = null;
      let paymentAmountsJson = null;
      let cardMachineIdsJson = null;

      if (multiPaymentSelections.length > 0) {
        paymentMethodsJson = multiPaymentSelections.map(p => p.methodName);
        paymentAmountsJson = multiPaymentSelections.map(p => p.amount);
        cardMachineIdsJson = multiPaymentSelections.map(p => p.cardMachineId || null);
      }
      ...
      const { data: order, error: orderError } = await supabase
        ...
        .insert({
          ...
          payment_methods: paymentMethodsJson, // Múltiplos pagamentos
          payment_amounts: paymentAmountsJson, // Valores de cada pagamento
          card_machine_ids: cardMachineIdsJson, // IDs das maquininhas
          delivery: isDelivery,
          delivery_fee: finalDeliveryAmount,
          discount_amount: totalDiscountAmount, // Salvar desconto total (manual + cupom)
  ...
  const finishOrderWithMultiPayment = async (payments: PaymentSelection[]) => {
    try {
      const monetarySubtotal = cart.reduce((sum, item) =>
        sum + (item.isRedeemedWithPoints ? 0 : (item.price * item.quantity)), 0
      );
      ...
      const totalMonetary = Math.max(0, monetarySubtotal + deliveryAmount - discountAmount);
      ...
      // Construir string de pagamento
      const paymentMethodsArray: string[] = [];
      if (pointsToRedeem > 0) {
        paymentMethodsArray.push("Fidelidade");
      }
      payments.forEach(p => paymentMethodsArray.push(p.methodName));
      const finalPaymentMethod = paymentMethodsArray.join(' + ');

      const initialStatus = getInitialStatus();

      // Preparar dados de múltiplos pagamentos
      const paymentMethodsJson = payments.map(p => p.methodName);
      const paymentAmountsJson = payments.map(p => p.amount);
      const cardMachineIdsJson = payments.map(p => p.cardMachineId || null);
      ...
      const { data: order, error: orderError } = await supabase
        ...
        .insert({
          ...
          payment_amounts: paymentAmountsJson,
          card_machine_ids: cardMachineIdsJson,
          delivery: isDelivery,
          delivery_fee: deliveryAmount,
          discount_amount: discountAmount,
          change_for: null, // Não usa troco em pagamento múltiplo
          cash_register_id: cashRegisterId,
          created_by: user?.id,
          delivery_address: isDelivery ? address : null,
          delivery_number: isDelivery ? number : null,
      ...
      // Atualização de estoque (mesma lógica que no finishOrder)
      const stockUpdatePromises = cart.map(async (item) => {
        let currentDbStock = 0;
        let tableName = "";
        let itemId = "";
        ...
        if (item.selectedVariation) {
          ...
          const isComposite = dbVariation?.is_composite || false;
          const newQuantity = isComposite
            ? currentDbStock - item.quantity
            : Math.max(0, currentDbStock - item.quantity);
      ...
      // Lógica de itens compostos (mesma do finishOrder)
      const compositeItemPromises = cart.map(async (item) => {
        if (!item.selectedVariation) return;
        ...
        const stockBeforeSale = item.stock_quantity || 0;
        ...
        const quantityNeeded = item.quantity - stockBeforeSale;
        const isRawMaterialVariation = !!raw_material_variation_id;
        const rawMaterialId = isRawMaterialVariation ? raw_material_variation_id : raw_material_product_id;
        const rawMaterialTable = isRawMaterialVariation ? "product_variations" : "products";
        ...
        if (!itemsError && orderItems && orderItems.length > 0) {
          await supabase
            ...
            .insert({
              ...
              raw_material_product_id: raw_material_product_id,
              raw_material_consumed: rawMaterialToConsume,
              variations_generated: unitsGenerated,
            });
        }
      });
      ...
      // Lógica de embalagens (mesma do finishOrder)
      const packagingUpdatePromises = cart.map(async (item) => {
        const { data: packagingLinks, error: packagingError } = await supabase
          .from("product_packaging_links")
          .select("packaging_id, quantity")
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-2rem)]">
          ...
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Label className="text-sm font-normal">Venda Rápida:</Label>
                <Select value={quickSaleMode} onValueChange={(value: any) => setQuickSaleMode(value)}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="fixed">Fixada</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </CardTitle>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 items-center">
                ...
                <div className="flex justify-end">
                  <Badge variant="default" className="text-lg font-bold" style={{ paddingTop: '1px', paddingBottom: '1px', paddingLeft: '7px', paddingRight: '7px' }}>
                    Total: R$ {totalMonetary.toFixed(2)}
                  </Badge>
                </div>
              </div>
              
              {/* Linha 2: Botões de Canal */}
              ...
              {/* Pontos a Resgatar (se houver) */}
              {pointsToRedeem > 0 && (
                <div className="flex items-center justify-between text-sm text-purple-600 font-medium bg-purple-50 p-2 rounded">
            ...
            {cart.length > 0 && (
              <>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <Label htmlFor="discount" className="cursor-pointer">Desconto?</Label>
                    <Input
                      id="discount"
                      type="number"
                      step="0.01"
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <Label htmlFor="multiPayment" className="text-sm text-muted-foreground cursor-pointer">
                          Múltiplos?
                        </Label>
                        <Switch
                          id="multiPayment"
                          ...
                          onCheckedChange={(checked) => {
                            ...
                            if (checked) {
                              // Limpar seleções individuais quando ativar modo múltiplo
                              setPaymentMethod(null);
                    ...
                    {!isMultiPaymentMode && (
                      <div className="grid grid-cols-2 gap-2">
                        {(["pix", "credito", "debito", "dinheiro", "reserva"] as PaymentMethod[]).map((method) => {
                        ...
                        {customPaymentMethods
                          .filter(pm => {
                            // Filtrar por canal
                            if (source === 'presencial' && !pm.allowed_channels.includes('presencial')) return false;
                            if (source === 'ifood' && !pm.allowed_channels.includes('ifood')) return false;
                            if (source === 'whatsapp' && !pm.allowed_channels.includes('whatsapp')) return false;
                            return true;
                          })
                          .map((method) => (
                            <Button
                    ...
                    {isMultiPaymentMode && multiPaymentSelections.length === 0 && (
                      <div className="p-4 bg-muted rounded-lg text-center text-sm text-muted-foreground">
                        Clique em <span className="font-semibold">Finalizar</span> para selecionar múltiplas formas de pagamento
                      </div>
                    )}
                    {isMultiPaymentMode && multiPaymentSelections.length > 0 && (
                      <div className="p-3 bg-muted rounded-lg space-y-2">
                        <p className="text-sm font-medium">Pagamentos Selecionados:</p>
                        {multiPaymentSelections.map((payment, index) => (
                          <div key={index} className="flex justify-between text-sm">
                            <span>{payment.methodName}{payment.cardMachineName ? ` (${payment.cardMachineName})` : ''}</span>
                  ...
                  {/* Seleção de máquina de cartão - AGORA COM BOTÕES */}
                  {!isMultiPaymentMode && ((paymentMethod === "credito" || paymentMethod === "debito") || 
                    (selectedPaymentMethodId && customPaymentMethods.find(pm => pm.id === selectedPaymentMethodId)?.card_machine_id)) && 
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {cardMachines.map(machine => (
                          <Button
                            key={machine.id}
                            variant={selectedCardMachineId === machine.id ? "default" : "outline"}
                            onClick={() => setSelectedCardMachineId(machine.id)}
                            className="flex items-center justify-center gap-2"
                          >
                            <CreditCard className="h-4 w-4 mr-2" />
                            {machine.name}
                ...
                <div className="space-y-2">
                  ...
                  {isDelivery && deliveryFee && (
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className={appliedCouponData?.freeShipping ? "line-through text-muted-foreground" : ""}>
                        R$ {deliveryAmount.toFixed(2)}
                      </span>
                      {appliedCouponData?.freeShipping && (
                        <Badge variant="secondary" className="text-xs ml-2">Grátis</Badge>
                      )}
                    </div>
                  )}
                  ...
                  {appliedCouponData && appliedCouponData.discountAmount > 0 && (
                    <div className="flex items-center justify-between text-sm text-green-600 font-medium">
                      <span>Cupom ({appliedCouponData.couponCode}):</span>
      ...
      {/* Dialog de Múltiplos Pagamentos */}
      <MultiPaymentDialog
        ...
        onConfirm={(payments) => {
          ...
          // Processar o pedido com múltiplos pagamentos
          finishOrderWithMultiPayment(payments);
        }}
      />
